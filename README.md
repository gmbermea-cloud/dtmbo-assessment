...
standardization, and shows a Dominant/Secondary blend result on-screen.

This is a separate product from COR3 — not part of that dashboard.

Results are shown on-screen only and are not saved anywhere server-side — there's no
database or spreadsheet write-back in this version.
...
2. Create an **API key** (Credentials → Create Credentials → API key). This only needs
   read access and is used server-side by `/api/items` — it's never exposed to the browser.
...
function — use `vercel dev` locally so the Items fetch works too.

## Deploy to Vercel

1. Push to GitHub and import the repo at [vercel.com/new](https://vercel.com/new).
2. Add `GOOGLE_SHEETS_API_KEY` and `GOOGLE_SHEET_ID` as environment variables in the
...
- On the 50th answer, results are scored and shown immediately, entirely client-side
  (`src/lib/scoring.js`). Nothing is sent back to a server at that point.
...
Saving/exporting results, PDF/download of results, authentication, an admin/aggregate
dashboard, and randomized question order.
